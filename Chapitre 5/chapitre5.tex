...
Une fois produite, les neutrinos se propagent presque sans absoption jusqu'à la terre - nous pouvons à présent les mesurer, mais cela reste un exercice non trivial (et n'est donc pas laissé en exercice pour le lecteur). Nous détectons approximativement la moitié du flux attendu.\\

Dans la résolution qui va suivre, nous supposerons que les neutrinos ont une masse (aussi faible soit-elle), et que les états propre de masse ne sont pas les états produits/absorbés lors de son intéraction avec la matière.

Supposons que nous ne travaillons qu'avec un neutrino $\nu$ dont nous ne préciserons pas la saveur. Soit $E$ l'énergie que possède le neutrino. Sa fonction d'onde peut-être décrite par l'onde plane
\begin{equation}
...
Nous supposons à présent l'existence de deux espèces de neutrino: le neutrino électronique et le neutrino $\ket{\nu_e}$ et $\ket{\nu_\mu}$. Le soleil produit $\ket{\nu_e}$, et nous ne sommes capable de détecter (sur Terre) que cette dernière. De plus, notons que lors de la propagation, le neutrino $\ket{\nu_1}$ possède une masse $m_1$ et le neutrino $\ket{\nu_2}$ possède une masse $m_2$.\\
...
\begin{equation}
    \begin{cases}
        \ket{\nu_e} = \cos\theta\ket{\nu_1}+\sin\theta\ket{\nu_2}\\
        \ket{\nu_\mu} = -\sin\theta\ket{nu_1}+\cos\theta\ket{\nu_2}
    \end{cases}
    \Leftrightarrow
    \begin{cases}
        \ket{\nu_1} = \cos\theta\ket{\nu_e}-\sin\theta\ket{\nu_\mu}\\
...
Empiquement, $\sin^22\theta \approx 0.85$ et $\Delta m^2 \approx 8 10^{-5} eV^2$.
