...
\begin{center}
\textbf{PREMIER POSTULAT}
\end{center}
A chaque instant, l'état d'un système physique est donné par un vecteur $\ket{\Psi}$ appartenant à un espace de Hilbert $\mathbb{H}$ normalisé tel que $\braket{\Psi|\Psi} = 1$, défini à une phase près : $\ket{\Psi} \thicksim e^{i\varphi}\ket{\Psi}$.
\end{minipage}}
\end{center}
\begin{remark}
...
\begin{center}
\textbf{SECOND POSTULAT}
\end{center}
A toute grandeur observable est associé un opérateur hermitien $A=A^\dag$.
\end{minipage}}
\end{center}
...
\begin{center}
\textbf{TROISIEME POSTULAT - Postulat de la Mesure}
\end{center}
Lors de la mesure de la grandeur physique $\mathcal{A}$ sur un système dans un état $\ket{\Psi}$ donne le résultat $a_n$, l'état dy système immédiatement après la mesure est la projection normée $\frac{P_n\ket{\Psi}}{\sqrt{\braket{\Psi|P_n|\Psi}}}$ de $\ket{\Psi}$ sur le sous-espace propre associé à $a_n$.
\end{minipage}}
\end{center}
...
\fbox{\begin{minipage}{15cm}
\begin{center}
\textbf{QUATRIEME POSTULAT - Évolution des États}
\end{center}
Soit $H(t)$ l'obserbable associée à l'énergie totale du système\footnote{Souvent, elle est appelée l'Hamiltonien du système}. Alors, l'évolution temporelle est donnée par l'équation
\begin{equation}
i\h \partial_t \ket{\Psi(t)} = H(t)\ket{\Psi(t)}
\label{Postulat 4}
\end{equation}
\end{minipage}}
\end{center}
